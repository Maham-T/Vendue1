<?php 
	...
	if(isset($_POST['search']))
	{
		...
		if($_POST['cust_id'] != ""){
			...
			$cnand .= " AND o.cust_id = '".$_POST['cust_id']."'";
		}
		if($_POST['domain_id'] != ""){
		...
		if($_POST['date_from'] != "" && $_POST['date_to'] != ""){
			...
			$cnand .= " AND DATE_FORMAT(o.datetime, '%Y-%m-%d') BETWEEN '".$_POST['date_from']."' AND '".$_POST['date_to']."'";
		}	
	}
	
	$sql="SELECT o.order_no,c.name, p.description,s.size,i.tnt_price,i.quantity,o.datetime,os.status,o.user_id,'RO' rtype FROM ".$tblprefix."orders o, ".$tblprefix."order_items i, ".$tblprefix."order_status os, ".$tblprefix."product p, ".$tblprefix."customer c,".$tblprefix."size s WHERE o.status_id = os.id and p.size = s.id and o.id = i.order_id and p.id = i.product_id and o.cust_id = c.id $and  
	union all 
			SELECT 
				o.order_no,
				c.name,
			...
			FROM
				...
				tnt_size s
			WHERE
				o.id = i.cn_id AND p.id = i.product_id
					AND p.size = s.id
					AND o.cust_id = c.id
					AND o.status = 5
					$cnand   "; 
        	...
        	<table  class="table table-striped table-bordered table-hover table-condensed" id="table">
              ...
              <tbody>
                ...
                <?php
					...
					$VATtotal   = $rs->fields["tnt_price"] + $VAT; 
